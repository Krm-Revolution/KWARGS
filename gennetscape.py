...
def generate_netflix_id():
    chars = string.ascii_letters + string.digits
    base = ''.join(random.choices(chars, k=32))
    timestamp = str(int(time.time() * 1000))
    hash_input = base + timestamp
    hash_value = hashlib.sha256(hash_input.encode()).hexdigest().upper()
    netflix_id = f"ct%3D{hash_value[:40]}%26ch%3DAQEAEAABABSAzNl7kIzXwAFwq7D7gFs_JQY49sFFDeY.%26v%3D3%26pg%3D{''.join(random.choices(chars, k=20))}"
    return netflix_id

def generate_secure_netflix_id(netflix_id):
    mac_hash = hashlib.sha256((netflix_id + str(time.time())).encode()).hexdigest()[:43]
    timestamp = str(int(time.time() * 1000))
    return f"v%3D3%26mac%3DAQEAEQABABT{mac_hash}%26dt%3D{timestamp}"

def generate_nfvdid():
    chars = string.ascii_letters + string.digits
    return f"BQFmAAEB{''.join(random.choices(chars, k=80))}"

def generate_optanon_consent():
    consent_id = hashlib.md5(str(time.time()).encode()).hexdigest()
    timestamp = datetime.now().strftime("%a+%b+%d+%Y+%H%%3A%M%%3A%S+GMT%2B0600+(Bangladesh+Standard+Time)")
    return f"isGpcEnabled=0&datestamp={timestamp}&version=202512.1.0&browserGpcFlag=0&isIABGlobal=false&hosts=&consentId={consent_id}&interactionCount=0&isAnonUser=1&landingPath=NotLandingPage&groups=C0001%3A1%2CC0002%3A1%2CC0003%3A1%2CC0004%3A1&AwaitingReconsent=false"

def generate_cookie_bundle():
    netflix_id = generate_netflix_id()
    secure_id = generate_secure_netflix_id(netflix_id)
    nfvdid = generate_nfvdid()
    optanon = generate_optanon_consent()
    expires = str(int(time.time()) + 31536000)
    secure_expires = str(int(time.time()) + 31536000)

    cookies = [
        f".netflix.com\tTRUE\t/\tFALSE\t{expires}\tnfvdid\t{nfvdid}",
        f".netflix.com\tTRUE\t/\tFALSE\t{expires}\tOptanonConsent\t{optanon}",
        f".netflix.com\tTRUE\t/\tTRUE\t{secure_expires}\tNetflixId\t{netflix_id}",
        f".netflix.com\tTRUE\t/\tTRUE\t{secure_expires}\tSecureNetflixId\t{secure_id}",
    ]
    ...
    cookie_dict = {
        ...
        "nfvdid": nfvdid,
        "OptanonConsent": optanon
    }
    return cookie_text, cookie_dict

def check_cookie_bundle(cookie_text, config):
...
def main():
    ...
    print(BANNER)
    print(f"\n{RED}{BOLD}╔══════════════════════════════════════════════════╗{RESET}")
    print(f"{RED}{BOLD}║{RESET}     {WHITE}NETFLIX COOKIE GENERATOR - HASH METHOD{RESET}       {RED}{BOLD}║{RESET}")
